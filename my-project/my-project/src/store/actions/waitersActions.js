...
//import { editUser as apiEditUser  } from '../../api';
...
//   }  
 ...
 export const editUser = (user) => (dispatch, getState) => {
  dispatch(setLoading(true)); console.log('editUser');
  // getUserList()
  //     .then((data) => dispatch(setUserList(data)))
  //     .finally(() => dispatch(setLoading(false)));
  //     };
...
export const addUser = (newForm) => (dispatch) => {
  ...
  dispatch(setLoading(true));
  newForm.id ? apiUpdateUser(newForm)
    .then((data) => dispatch(updateUser(data))).finally(() => dispatch(setLoading(false))) :
  apiCreateUser({...newForm,  id: Date.now()})
      .then((data) => dispatch(createUser(data))).finally(() => dispatch(setLoading(false)))
